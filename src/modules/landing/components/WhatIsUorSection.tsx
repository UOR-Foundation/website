...
const UorDiagramCompact = () => (
  <div className="mx-auto w-full max-w-[54rem] rounded-[1.75rem] border border-[hsl(var(--section-dark-foreground)/0.1)] bg-[hsl(var(--section-dark))] px-[clamp(1.5rem,2vw,2.75rem)] py-[clamp(2.25rem,3vw,3.75rem)] lg:min-h-[clamp(26rem,31vw,33rem)]">
        ...
        <div className="py-golden-lg grid grid-cols-1 items-start gap-[clamp(2.5rem,4vw,5.5rem)] animate-fade-in-up opacity-0 lg:grid-cols-[minmax(0,0.618fr)_minmax(0,1fr)]" style={{ animationDelay: "0.11s" }}>
          <div>
            <p className="font-body leading-[1.75] md:leading-[1.85] text-fluid-lead">
              <span className="text-foreground/90 font-medium">Every system invents its own IDs. The moment data crosses a boundary, those IDs collide, break, or vanish.</span>{" "}
              <span className="text-foreground/70 font-normal">UOR derives the address from the content itself. Same data, same address — everywhere, forever.</span>
            </p>
            <p className="mt-golden-md text-foreground/70 font-body leading-[1.75] md:leading-[1.85] text-fluid-lead">
              No registry. No middleman. No single point of failure. Data carries its own identity, verifiable on any system, across any network.
            </p>
