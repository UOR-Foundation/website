...
describe("T0: Synergy Structure", () => {
  ...
  it("every chain has matching bridges (projections - 1)", () => {
    for (const chain of SYNERGY_CHAINS) {
      expect(chain.bridges.length).toBe(chain.projections.length - 1);
    }
  });

  it("15 synergy chains are defined", () => {
    expect(SYNERGY_CHAINS.length).toBe(15);
  });
...
describe("T3: Synergy Analysis Engine", () => {
  ...
  it("analyzeSynergies reports correct chain count", async () => {
    await setup();
    const analysis = analyzeSynergies(testInput);
    expect(analysis.stats.totalChains).toBe(15);
  });
