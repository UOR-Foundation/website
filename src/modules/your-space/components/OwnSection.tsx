/**
 * Your Space — "Own" Section with drag-and-drop reordering
...
export const OwnSection = ({ isDark, searchQuery, setSearchQuery, votes, onVote }: OwnSectionProps) => {
  ...
  const cards: Record<string, React.ReactElement> = {
    security: (
      <SpaceCard
        title="Security"
        icon={<Shield className="text-foreground" size={16} />}
        isDark={isDark}
        moduleSlug="shield"
        editPanel={
          <div className="space-y-3">
            <p className="text-muted-foreground text-sm font-body">Configure your cryptographic preferences:</p>
            {[
              ...
              <div key={field.key}>
                <label className="text-muted-foreground text-sm font-body block mb-1">{field.label}</label>
                <select
                  value={securityEditing[field.key]}
                  onChange={(e) => setSecurityEditing(prev => ({ ...prev, [field.key]: e.target.value }))}
                  className="w-full bg-muted border-border text-foreground border rounded-lg px-3 py-2 font-body text-sm focus:outline-none transition-colors"
                >
        ...
        expandedContent={
          <div className="space-y-3">
            <h4 className="text-foreground font-body text-sm font-semibold">Security Audit</h4>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={item.label} className="p-3 rounded-lg border border-border bg-muted/50">
                  <p className="text-muted-foreground text-xs font-body">{item.label}</p>
                  <p className="text-foreground text-base font-body font-semibold">{item.value}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-3.5">
          {[
            ...
            <div key={label as string} className="flex items-center justify-between">
              ...
              {typeof value === "string" ? <span className="text-foreground font-body text-sm font-medium">{value}</span> : value}
            </div>
          ))}
        </div>
      </SpaceCard>
    ...
    identity: (
      <SpaceCard
        title="Identity"
        icon={<User className="text-foreground" size={16} />}
        isDark={isDark}
        moduleSlug="identity"
        expandedContent={
          <div className="space-y-3">
            <h4 className="text-foreground font-body text-sm font-semibold">Identity Graph</h4>
            <p className="text-muted-foreground text-sm font-body leading-relaxed">
    ...
    assets: (
      <SpaceCard
        title="Assets"
        icon={<Wallet className="text-foreground" size={16} />}
        isDark={isDark}
        moduleSlug="store"
        editPanel={
          <div className="space-y-3">
            <p className="text-muted-foreground text-sm font-body">Portfolio display settings:</p>
            <label className="flex items-center gap-2 text-muted-foreground text-sm font-body cursor-pointer">
        ...
        expandedContent={
          <div className="space-y-3">
            <h4 className="text-foreground font-body text-sm font-semibold">Asset Provenance</h4>
            <p className="text-muted-foreground text-sm font-body leading-relaxed">
      ...
      >
        <div className="space-y-3">
          <div className="flex justify-end">
            <button onClick={() => setHideNumbers(!hideNumbers)} className="text-muted-foreground hover:text-foreground transition-colors">
              {hideNumbers ? <EyeOff size={14} /> : <Eye size={14} />}
            </button>
          </div>
          <div className="grid grid-cols-4 gap-2 text-xs font-body font-medium text-muted-foreground pb-2 border-b border-border">
            <span>TYPE</span>
          ...
          {[
            ...
            <div key={row.type} className="grid grid-cols-4 gap-2 items-center font-body text-sm">
              ...
              <span className="text-foreground text-right font-medium">{hideNumbers ? "•••••" : row.usd}</span>
              <span className="text-foreground text-right">{hideNumbers ? "••••" : row.btc}</span>
              <span className={`${row.positive ? "text-emerald-600 dark:text-emerald-400" : "text-red-500"} text-right font-medium`}>
                {hideNumbers ? "••••" : row.change}
              </span>
            </div>
          ))}
          <div className="grid grid-cols-4 gap-2 items-center font-body text-sm pt-2 border-t border-border">
            <span className="text-foreground font-semibold">Total</span>
            <span className="text-foreground font-bold text-right">{hideNumbers ? "•••••" : "83,555"}</span>
            <span className="text-foreground font-bold text-right">{hideNumbers ? "••••" : "1.29"}</span>
            <span className="text-emerald-600 dark:text-emerald-400 text-right font-bold">{hideNumbers ? "••••" : "+1.1%"}</span>
          </div>
        </div>
      </SpaceCard>
    ),
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-foreground font-body text-xl font-semibold">Own Your Data</h2>
        <div className="relative w-80">
          ...
          <input
            ...
            className="w-full bg-muted border-border text-foreground border rounded-lg py-2.5 pl-10 pr-10 font-body text-sm focus:outline-none transition-all placeholder:text-muted-foreground"
          />
          {searchQuery.trim() && (
            <button className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground">
