 ...
 * ═══════════════════════════════════════
 *
 * Performance-optimized: URL-keyed page cache, memoized markdown
 * components, module-scoped IconBtn. No iframes — every page becomes
 * a first-class hologram document.
 */
...
/* ── Module-scoped IconBtn — never recreated ─────────────────── */
function IconBtn({ onClick, disabled, children, title, active }: {
  onClick?: () => void;
  disabled?: boolean;
...
/* ── URL-keyed page cache — instant revisits ─────────────────── */
...
export default function HologramBrowser({ onClose, onSendToLumen }: BrowserProps) {
  ...
  // Use refs for navigate callback to avoid stale closures
  ...
  /* ── Memoized ReactMarkdown components — stable reference ──── */
  const markdownComponents = useMemo(() => ({
    a: ({ href, children, ...props }: any) => (
      <a
        ...
        onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.borderBottomColor = P.gold; }}
        onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.borderBottomColor = "hsla(38, 30%, 55%, 0.2)"; }}
      >
        {children}
      </a>
    ),
    ...
    blockquote: ({ children }: any) => (
      <blockquote
        ...
        style={{ borderLeft: `2px solid ${P.gold}`, background: "hsla(38, 15%, 20%, 0.2)", color: "hsl(38, 12%, 75%)" }}
      >
        {children}
      </blockquote>
    ),
    code: ({ children, className }: any) => {
      ...
      if (isBlock) {
        return (
          <pre
            ...
            style={{ background: "hsla(25, 10%, 6%, 0.6)", border: `1px solid ${P.border}`, color: "hsl(38, 10%, 78%)" }}
          >
            <code>{children}</code>
          </pre>
        );
      ...
      return (
        <code
          ...
          style={{ background: "hsla(38, 10%, 20%, 0.3)", color: P.gold }}
        >
          {children}
        </code>
      );
    ...
    th: ({ children }: any) => (
      <th className="text-left px-3 py-1.5 text-[11px] font-medium uppercase tracking-wider" style={{ borderBottom: `1px solid ${P.border}`, color: P.goldMuted }}>
        {children}
      </th>
    ),
    td: ({ children }: any) => (
      <td className="px-3 py-1.5 text-[12px] font-light" style={{ borderBottom: `1px solid hsla(38, 10%, 25%, 0.15)`, color: "hsl(38, 10%, 78%)" }}>
        {children}
      </td>
    ),
  }), [handleLinkClick]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <IconBtn onClick={() => page && navigate(page.url, true)} disabled={loading || !page}>
        ...
        <div className="flex-1 overflow-y-auto lumen-scroll">
              ...
              <div className="flex flex-wrap gap-1.5 justify-center">
                {[
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => { e.currentTarget.style.background = P.surfaceHover; e.currentTarget.style.borderColor = P.borderFocus; }}
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center h-full gap-3">
              ...
              <p className="text-[12px] font-light" style={{ color: P.textMuted }}>Rendering…</p>
            </div>
          )}
          ...
          {/* Page Content — uses memoized components */}
          {page && !loading && (
            <article className="px-8 md:px-14 py-8 max-w-[1000px] mx-auto w-full" style={{ color: P.text }}>
