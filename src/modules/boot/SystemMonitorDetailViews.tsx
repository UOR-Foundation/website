  ...
  return (
    <div className="space-y-1">
      ...
      <DetailTable rows={[
        ...
        { label: "WASM Attempts", value: diag.attempts },
        ...(diag.lastError ? [{ label: "Last WASM Error", value: <span className="text-red-400">{diag.lastError}</span> }] : []),
      ]} />
    </div>
  );
}
...
function ModulesDetail({ receipt, onBack }: DetailViewProps) {
  ...
  useEffect(() => {
    import("@/modules/uns/core/pruning-gate").then(({ pruningGate }) => {
      const r = pruningGate();
      setPruningData({
