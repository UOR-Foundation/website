...
import { ExternalLink, ChevronLeft, ChevronRight, Fingerprint, ShieldCheck, Copy, Check, Table, BarChart3, Boxes } from "lucide-react";
...
type QuickLookTab = "preview" | "blueprint";

export default function QuickLookModal({ items, currentIndex, onClose, onNavigate }: Props) {
  const [activeTab, setActiveTab] = useState<QuickLookTab>("preview");
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-w-[70vw] w-full max-h-[80vh] flex flex-col gap-0 p-0 overflow-hidden border-border/30 bg-background/95 backdrop-blur-xl shadow-2xl">
        ...
        {/* Tab bar */}
        <div className="flex items-center gap-0 px-5 border-b border-border/20 flex-shrink-0">
          <button
          ...
          <button
            ...
            className={`px-3 py-2 text-[11px] font-medium border-b-2 transition-colors flex items-center gap-1.5 ${
              activeTab === "blueprint"
                ? "border-primary text-foreground"
                : "border-transparent text-muted-foreground/50 hover:text-foreground/70"
            }`}
        ...
        <div className="flex-1 overflow-auto p-5 min-h-0">
          {activeTab === "preview" ? (
            ...
            <BlueprintPanel uorAddress={item.uorAddress || `urn:uor:local:${item.id}`} />
          )}
        </div>
      </DialogContent>
    </Dialog>
